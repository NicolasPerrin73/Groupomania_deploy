...
const Post = ({ deletedPost, setDeletedPost, post_id, content, img_url, created_date, user_id, nom, prenom, picture_url, current_user_id, current_user_is_admin }) => {
  ...
  useEffect(() => {
    if (IsConfirmed === true) {
      ...
      axios
        .delete(`https://minidev.fr:5443/api/post/${post_id}`, {
          headers: {
  ...
  useEffect(() => {
    if (likeClick === true) {
      ...
      axios
        .post(
          `https://minidev.fr:5443/api/post/${post_id}/like`,
          { like: likeValue },
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios
      .get(`https://minidev.fr:5443/api/post/${post_id}/like`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios
      .get(`https://minidev.fr:5443/api/post/${post_id}/like/user`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
